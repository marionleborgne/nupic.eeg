...
if __name__ == "main":
    ...
    def train_channel(i):
        ...
        [model.run({"channel1": float(r)}).inferences['anomalyScore'] for r in dataset.data[i]]
        model.disableLearning()
        model.save("%s.channel.%s.model")

    p = Pool(16)
    pool.map(train_channel, range(len(dataset.channels)))

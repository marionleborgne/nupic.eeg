...
for label in labels:
    for channel in channels:
        ...
        filtered_data[key].append(header)
        # add unit for nupic
        unit = []
        unit.append("float")
        filtered_data[key].append(unit)
        # timestamp stuff that won't be useful in this case so it's set to ""
        timestamp = []
        timestamp.append("")
        filtered_data[key].append(timestamp)
...
for rawRow in csvReader:
    ...
    if rowCount % sampling_rate == 0:
        ...
        for i in range(len(row)):
            channel = channels[i]
            key = "%s_%s" % (label, channel)
            sampled_data[key].append(float(row[i]))
...
for label in labels:
    for channel in channels:
        ...
        b = 20
        beta = [5,16]
        x_filtered = smooth.smooth(sampled_data[key],b,window_len)

        for value in x_filtered:
            newRow = []
            newRow.append(value)
            filtered_data[key].append(newRow)

        fileName = "formatted_data/%s.csv" % key
        with open(fileName, 'wb') as csvfile:
            writer = csv.writer(csvfile, delimiter=',')
            writer.writerows(filtered_data[key])